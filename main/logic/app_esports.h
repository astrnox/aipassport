...
#define APP_ESPORT_GOLD_POINTS  12  // 经济曲线保留的采样点上限
...
// 选手位置。接口用 "top"/"jungle"/"mid"/"bottom"/"support" 表达，这里归一成枚举，
// 中文名由界面层映射，模型层不携带展示文案。
typedef enum {
    APP_ROLE_TOP = 0,
    APP_ROLE_JUNGLE,
    APP_ROLE_MID,
...
// 单场详情里的一名选手：阵容来自 window 的 gameMetadata，经济与 KDA 来自最新的有效采样帧。
typedef struct {
    app_esport_role_t role;
    char champion[APP_ESPORT_CHAMP_LEN];
    char player[APP_ESPORT_PLAYER_LEN];
    int  gold;
    int  kills;
    int  deaths;
    int  assists;
...
// 单场对局详情（二级下钻）。valid 只表示"拿到了阵容"，经济/选手数据需要采样帧，
// 取不到时对应字段保持 0，由界面层降级标注，不伪造数据。
typedef struct {
    bool valid;
    char match_id[32];        // 归属比赛；与当前进入的比赛不一致时视为过期
    int  game_number;         // 第几局（1 起）
    char game_state[16];      // 接口原文：inProgress / completed / unstarted
    app_esport_player_t team_a[APP_ESPORT_TEAM_PLAYERS];   // 对应赛程里的 team_a
    app_esport_player_t team_b[APP_ESPORT_TEAM_PLAYERS];
    int  gold_a[APP_ESPORT_GOLD_POINTS];                   // 总经济采样，已按 team_a/team_b 归一
    int  gold_b[APP_ESPORT_GOLD_POINTS];
    int  gold_points;         // 有效采样点数，< 2 时界面不画曲线
    int  fetched_utc;
} app_esport_detail_t;
...
typedef struct {
    ...
    bool valid;           // 是否有可用缓存
    app_esport_detail_t detail;   // 最近一次下钻的对局详情，随赛程缓存一起持久化
} app_esport_cache_t;
...
// 接口原文位置名 -> 枚举；未知返回 APP_ROLE_UNKNOWN。
