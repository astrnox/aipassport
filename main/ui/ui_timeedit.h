...
//   短按 UP / DOWN  当前字段 -1 / +1，到端点后循环到另一端（12 月再按 +1 回到 1 月）
//   长按 UP / DOWN  当前字段 -step / +step（大步），在 min..max 间夹紧
//   短按 OK         跳到下一个字段；在最后一个字段上等同保存
//   长按 OK         保存并关闭
// step 是长按步长；数值字段默认给 10，符合"长按加十"的直觉。选项字段（names 非空）
// 无论长短按都按 1 步在选项间循环。提示条会写出当前字段真实的长按步长，不会说的与做
// 的不一致。
#pragma once
