...
| 14 | 摇卦与每日一签 | 离线生成卦象与每日内容，展示卦名与卦辞；同一天内容稳定不重复。 |
...
需要出图的界面：主页、万年历、时间进度、秒表与计时器、番茄钟、提醒列表、作息时间轴、作息配置、电子工牌、动态口令、摇卦、硬件自检、赛事赛程、积分榜、战队详情、对局详情、首次引导与时间设置、配网引导、系统设置、快捷面板、空状态与离线状态。对应的可交互原型见配套文件 `AI-Passport-界面原型.html`，原型已覆盖上述全部界面。
...
AI 语音对话与电台等需要持续联网且算力要求高的能力；蓝牙 HID 遥控（PPT、短视频等）能力；多设备联动的社交玩法；天气等第三方内容聚合；跨设备数据同步与账号体系；赛事数据的三级扩展信息；需要每日维护的习惯打卡类功能。
